...
int count_branches_to_key(Tree* root, int key) {
    if (root == nullptr) { // базовый случай: дерево пустое
        return 0;
    }
    if (root->key == key) { // базовый случай: ключ найден
        cout << root->key << " " << root->text << endl;
        return 1;
    }
    if (key < root->key) { // рекурсивный случай: ключ меньше, чем ключ корня
        cout << root->key << " " << root->text << endl;
        return 1 + count_branches_to_key(root->Left, key);
    }
    else { // рекурсивный случай: ключ больше, чем ключ корня
        cout << root->key << " " << root->text << endl;
        return 1 + count_branches_to_key(root->Right, key);
    }
}