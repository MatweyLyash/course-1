...
template<class Data, class Key>
int HashTable<Data, Key>::universal_hash(const Key& key)
{
	std::srand(std::time(NULL));
	int a = std::rand() % capacity;
	int b = std::rand() % (capacity - 1) + 1;
	return static_cast<int>(key * a + b) % capacity;
}

template<class Data, class Key>
int HashTable<Data, Key>::universal_hash(const Key& key, const int& max_size)
{
	std::srand(std::time(NULL));
	int a = std::rand() % max_size;
	int b = std::rand() % (max_size - 1) + 1;
	return static_cast<int>(key * a + b) % max_size;
}
...
void HashTable<Data, Key>::resize(const int& new_capacity)
{
	...
	for (int i = 0; i < capacity; i++)
	{
		if (table[i] != nullptr)
		{
			int index = universal_hash(table[i]->key, new_capacity);
...
{
	int index = universal_hash(key), i = 0;
	while (i < capacity)
	{
